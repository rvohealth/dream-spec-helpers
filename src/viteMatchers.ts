...
export default function provideDreamViteMatchers(Dream: any) {
  expect.extend({
    ...
    toEqualClockTime(received: any, expected: any) {
      if (!(received?.constructor?.name === "ClockTime")) {
        return {
          pass: false,
          message: () =>
...
export function expectMatchingDreamModels(
  ...
  if (expected === undefined) {
    ...
    message = () =>
      ERROR_COLOR(`expected is ${expected} but should be an instance of Dream`)
    ...
    message = () =>
      ERROR_COLOR(`received is ${received} but should be an instance of Dream`)
